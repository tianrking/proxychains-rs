...
fn native_readiness_and_failure_cleanup() {
    ...
    assert!(
        !marker.exists(),
        "loaded DLL with failed initialization must never start payload"
    );
    ...
    let exact_name = std::path::Path::new(&fixture)
        .file_name()
        .unwrap()
        .to_string_lossy();
    ...
    assert!(
        alive,
        "failed attach must not terminate an existing process"
    );
    assert!(
        retry.is_ok(),
        "corrected configuration must be retryable: {retry:?}"
    );
    assert!(by_name.is_ok(), "unique executable attachment: {by_name:?}");
    assert!(
        ambiguous.is_err(),
        "multiple processes require explicit PID"
    );
    ...
    verify_dns_queryex(&good, &fixture, &config);
...
fn verify_dns_queryex(injector: &ProxychainsInjector, fixture: &str, config: &std::path::Path) {
    std::fs::write(
        config,
        "strict_chain\nproxy_dns\n[ProxyList]\nsocks5 127.0.0.1 9\n",
    )
    ...
    let info = ProcessInfo {
        ...
        name: None,
        command: fixture.into(),
        args: vec!["dns-queryex".into()],
    };
    assert_eq!(injector.spawn_inject_wait(&info).unwrap(), 23);
}

fn verify_tcp_connectex(
    injector: &ProxychainsInjector,
    fixture: &str,
    config: &std::path::Path,
    mode: &str,
    ...
    let server = std::thread::spawn(move || {
        ...
        assert!(
            header.starts_with(b"CONNECT 192.0.2.123:443 HTTP/1.0\r\n"),
            "{}",
...
fn verify_tcp_routing(injector: &ProxychainsInjector, fixture: &str, config: &std::path::Path) {
    ...
    let info = ProcessInfo {
        ...
        name: None,
        command: fixture.into(),
        args: vec!["tcp".into(), "192.0.2.123:443".into()],
    };
    ...
    assert_eq!(
        status.unwrap(),
        23,
        "injected TCP tunnel must preserve payload"
    );
