...
- On Linux, `dup`, `dup2` and `dup3` preserve the shared SOCKS5 association
  and remove replaced descriptor state. macOS dyld interposition intentionally
  leaves descriptor duplication on the native path until a recursion-safe
  implementation is available. Descriptor passing, sockets inherited across fork/exec, Windows socket duplication, direct syscalls, io_uring,
