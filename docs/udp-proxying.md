...
- On Unix, `dup`, `dup2`, Linux `dup3` and `fcntl(F_DUPFD*)` preserve the shared
  SOCKS5 association and remove replaced descriptor state. Descriptor passing,
  sockets inherited across fork/exec, Windows socket duplication, direct syscalls, io_uring,
