...
| Unix socket lifecycle and event-loop compatibility | Premature close and flag loss fixed; Unix `dup`/`dup2`, Linux `dup3` and Unix `fcntl(F_DUPFD*)` now preserve UDP association state; event-loop compatibility pending | Linux/macOS native preload fixtures cover duplication; replacing an fd still does not preserve epoll/kqueue registrations |
