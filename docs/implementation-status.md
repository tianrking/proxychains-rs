...
| Unix socket lifecycle and event-loop compatibility | Premature close and flag loss fixed; Linux `dup`/`dup2`/`dup3` now preserve UDP association state; event-loop compatibility pending | Linux/macOS compile checks pass; Linux native preload fixture covers duplication; macOS descriptor duplication remains native until recursion-safe dyld interposition is implemented; replacing an fd still does not preserve epoll/kqueue registrations |
