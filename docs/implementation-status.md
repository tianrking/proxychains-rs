...
| UDP and IPv6 transport | SOCKS5 UDP transport, explicit forwarder, opt-in transparent UDP hooks, multi-node association failover, synchronous, IOCP and overlapped completion-routine Windows `WSASendMsg`/`WSARecvMsg`/`WSASendTo`/`WSARecvFrom`, ConnectEx cancellation signaling, and a proxy-backed RIO extension table for registered buffers, request/completion queues, `RIOSend`/`RIOReceive` and completion dequeue | Windows native UDP fixture passes, including IPv6 relay, domains, authentication, vectored I/O, IOCP and completion-routine delivery, cancellation, extension-pointer calls, RIO table/buffer/queue/send-completion behavior, successful `RIOSendEx` traffic through a live SOCKS5 relay, and a real Quinn+h3 HTTP/3 GET/response through a SOCKS5 relay; RIO receive-address/control metadata, browser and MsQuic remain uncertified |
