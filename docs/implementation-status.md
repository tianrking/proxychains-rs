...
| UDP and IPv6 transport | SOCKS5 UDP transport, explicit forwarder, opt-in transparent UDP hooks, and per-socket UDP route groups implemented | Windows native UDP fixture passes, including IPv6 relay, domains, authentication, vectored I/O and failures; see [UDP scope](udp-proxying.md); async Windows UDP and general QUIC remain unsupported |
