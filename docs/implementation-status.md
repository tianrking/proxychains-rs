...
| DNS mapping and configuration lifecycle | Core fixes implemented; IPv4 and IPv6 localnet bypass rules supported; Windows `GetAddrInfoExA` and `GetAddrInfoExW` now participate in proxy-DNS fake-IP mapping | Custom subnet, concurrency, exhaustion, IPv6 CIDR bypass and explicit missing path tests; application-specific asynchronous resolver behavior remains subject to native validation |
