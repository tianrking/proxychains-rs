...
| DNS mapping and configuration lifecycle | Core fixes implemented; IPv4 and IPv6 localnet bypass rules supported; synchronous and callback/event-based asynchronous Windows `GetAddrInfoExA/W`, synchronous `DnsQuery_A/W/UTF8`, and callback-based `DnsQueryEx`, participate in proxy-DNS fake-IP mapping | Custom subnet, concurrency, exhaustion, IPv6 CIDR bypass and explicit missing path tests; `GetAddrInfoEx` and `DnsQueryEx` contexts are released at their documented completion points; legacy `DnsQuery_A/W` async flags remain system-resolved because those parameters are reserved |
...
| UDP and IPv6 transport | SOCKS5 UDP transport, explicit forwarder, opt-in transparent UDP hooks, multi-node association failover, synchronous, IOCP and overlapped completion-routine Windows `WSASendMsg`/`WSARecvMsg`/`WSASendTo`/`WSARecvFrom`, ConnectEx cancellation signaling, and a proxy-backed RIO extension table for registered buffers, request/completion queues, `RIOSend`/`RIOReceive`, completion dequeue and event notification | Windows native UDP fixture passes, including IPv6 relay, domains, authentication, vectored I/O, IOCP and completion-routine delivery, cancellation, extension-pointer calls, RIO table/buffer/queue/send-completion behavior, successful `RIOSendEx` traffic and `RIOReceiveEx` source-address delivery through a live SOCKS5 relay, event notification, and a real Quinn+h3 HTTP/3 GET/response through a SOCKS5 relay; RIO control metadata, IOCP notifications, browser and MsQuic remain uncertified |
...
the remaining possibilities include an un-injected network-process path or a
browser-specific registered-I/O path. This observation is diagnostic only and
does not certify browser HTTP/3 support or the untested RIO receive metadata
and provider-specific notification paths.
