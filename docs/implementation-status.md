...
| UDP and IPv6 transport | SOCKS5 UDP transport, explicit forwarder, opt-in transparent UDP hooks, multi-node association failover, synchronous and IOCP Windows `WSASendMsg`/`WSARecvMsg`, Windows IOCP `WSASendTo`/`WSARecvFrom`, ConnectEx cancellation signaling, and explicit RIO bypass rejection implemented | Windows native UDP fixture passes, including IPv6 relay, domains, authentication, vectored I/O, IOCP completion, cancellation, extension-pointer calls, RIO `WSAEOPNOTSUPP` rejection and a real Quinn+h3 HTTP/3 GET/response through a SOCKS5 relay; native readiness exercises ConnectEx event/IOCP close-cancellation; browser and MsQuic remain uncertified |
