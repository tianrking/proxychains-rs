...
| DNS mapping and configuration lifecycle | Core fixes implemented; IPv4 and IPv6 localnet bypass rules supported; synchronous and callback/event-based asynchronous Windows `GetAddrInfoExA/W`, plus synchronous `DnsQuery_A/W`, participate in proxy-DNS fake-IP mapping | Custom subnet, concurrency, exhaustion, IPv6 CIDR bypass and explicit missing path tests; event contexts are released through `GetAddrInfoExOverlappedResult`; asynchronous `DnsQuery_A/W` still use the original resolver |
