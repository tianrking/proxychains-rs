...
| DNS mapping and configuration lifecycle | Core fixes implemented; IPv4 and IPv6 localnet bypass rules supported; synchronous and callback/event-based asynchronous Windows `GetAddrInfoExA/W`, plus synchronous and callback-based `DnsQueryEx`, participate in proxy-DNS fake-IP mapping | Custom subnet, concurrency, exhaustion, IPv6 CIDR bypass and explicit missing path tests; `GetAddrInfoEx` and `DnsQueryEx` contexts are released at their documented completion points; legacy `DnsQuery_A/W` async mode still uses the original resolver |
