...
| Unix socket lifecycle and event-loop compatibility | Premature close and flag loss fixed; Unix `dup`/`dup2`/Linux `dup3` now preserve UDP association state; event-loop compatibility pending | Linux/macOS compile checks pass; native preload fixture added but not run locally; replacing an fd still does not preserve epoll/kqueue registrations |
