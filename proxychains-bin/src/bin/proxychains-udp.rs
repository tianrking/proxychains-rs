...
use clap::Parser;
use proxychains::{
    config::{ProxyData, ProxyType},
    proxy::{TargetAddress, UdpAssociation},
    ConfigParser,
};
...
fn main() -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        args.target.port() != 0 && !args.target.ip().is_unspecified(),
        "target must be a concrete IP and nonzero port"
    );
    ...
    eprintln!(
        "UDP {} -> {} through SOCKS5; maximum 64 clients, 60s idle expiry",
        socket.local_addr()?,
        args.target
    );
    run(
...
fn session(
    ...
    let association =
        UdpAssociation::connect(&proxy, Duration::from_secs(3), Duration::from_secs(3))?;
    ...
    loop {
        for _ in 0..32 {
            match receiver.try_recv() {
                Ok(data) => {
                    association.send_to(&destination, target.port(), &data)?;
                }
...
mod tests {
    ...
    fn two_clients_have_independent_relay_sessions() {
        ...
        let mock = std::thread::spawn(move || {
            ...
            while workers.len() < 2 {
                ...
                workers.push(std::thread::spawn(move || {
                    ...
                    let (size, peer) = relay.recv_from(&mut packet).unwrap();
                    relay.send_to(&packet[..size], peer).unwrap();
                    assert_eq!(
                        control.read(&mut [0]).unwrap(),
                        0,
                        "shutdown releases association"
                    );
                }));
            }
        ...
        a.send_to(b"same-id:client-a", addr).unwrap();
        b.send_to(b"same-id:client-b", addr).unwrap();
        let mut buf = [0; 128];
        let (size, _) = a.recv_from(&mut buf).unwrap();
        assert_eq!(&buf[..size], b"same-id:client-a");
        let (size, _) = b.recv_from(&mut buf).unwrap();
        assert_eq!(&buf[..size], b"same-id:client-b");
